 ...
 */
define(['jquery', 'underscore', 'backbone', 'model/lookupcriteria', 'model/cartItem', 'model/checkListGroup', 'model/checkListItem', 'model/binManager', 'model/boxItem', 'model/pick', 'model/pack', 'collection/cart', 'view/base/printer', 'view/common/global', 'view/common/service', 'view/common/shared', 'view/common/preference', 'view/common/method', 'view/common/enum', 'view/base/numericpad', 'view/binManager/binManager', 'view/lookup/itemLookup/itemLookup', 'view/lookup/lookup', 'view/pick/card', 'view/pick/cardList', 'view/pick/cartItem', 'view/pick/cartSkippedItem', 'view/pick/checkListGroup', 'view/pick/checkListItem', 'view/pick/itemSetting', 'text!template/pick/pick.tpl.html', 'jqueryhammer'], function($, _, Backbone, LookupCriteriaModel, BinManagerModel, CartItemModel, CheckListGroupModel, CheckListItemModel, BoxItemModel, PickModel, PackModel, CartCollection, Printer, Global, Service, Shared, Preference, Method, Enum, NumericPadView, BinManagerView, ItemLookupView, LookupView, CardView, CardListView, CartItemView, CartSkippedItemView, CheckListGroupView, CheckListItemView, ItemSettingView, PickTemplate) {
    ...
    var PickView = Backbone.View.extend({
        ...
        ChangeCardSize: function(skippedItemID) {
            if (Global.IsPrePackMode) {
                ...
                try {
                    var detailScroll = new IScroll('#itemSettingBody' + skippedItemID, {
                        vScrollbar: false,
                        scrollY: true
                    });
                } catch (e) {}
        ...
        LoadCheckListLookup: function(listMode) {
            ...
            switch (listMode) {
                ...
                case Enum.ListMode.ServiceTypes:
                    ...
                    if (this.CurrentItem.get("CarrierDescription") == "Manual") lookupModel.set({
                        CarrierCode: this.CurrentItem.get("DescriptionCarrierCode")
                    });
        ...
        PickItemFromItemLookup: function(item) {
            ...
            if (existingItem) {
                if (this.CurrentItem.get('UPCCode') == upcCode && this.CurrentItem.get('UnitMeasureCode') == unitMeasureCode) {
                    Shared.BeepSuccess();
        ...
        PopulateCart: function(items) {
            if (items && items.length > 0) {
                ...
                _.each(items, function(current) {
                    ...
                    if (current.UPCCode != null && current.UPCCode != "") card.set({
        ...
        PrepareBin: function(items) {
            if (items && items.length == 1 && items[0].QuantityToPick == 1) this.model.set({
                HasOnlyOneItem: true
            });
            if (items && items.length > 0) {
        ...
        UpdateServiceType: function(itemModel) {
            if (itemModel.model.get('CarrierDescription') == "Manual") this.CurrentItem.set({
                ServiceCode: null
            });
            else {
                this.CurrentItem.set({
        ...
        WireCardEvents: function(skippedItemID, isSkippedItem) {
            ...
            if (isSkippedItem) {
                ...
                if (Preference.PickIsPromptBinManager) {
                    Shared.AddRemoveHandler('#binColIcon' + skippedItemID, 'tap', function(e) {
                        self.buttonBinManager_tap(e);
                    });
                    Shared.AddRemoveHandler('#binCol' + skippedItemID, 'tap', function(e) {
                        self.buttonBinManager_tap(e);
                    });
                } else {
                    Shared.AddRemoveHandler('#binColIcon' + skippedItemID, 'tap', function(e) {
                ...
                if (!$._data($('#currentCardContainer > div').get(0), 'events')) {
                    Shared.AddRemoveHandler('#currentCardContainer > div', 'tap', function() {
                        self.RotateContainerCard("");
                    });
                }
            }
        ...
        WireEvents: function() {
            ...
            Shared.AddRemoveHandler('#buttonEditDetails', 'tap', function(e) {
            ...
            Shared.AddRemoveHandler('#buttonBackBoxDetails', 'tap', function(e) {
                self.buttonBackBoxDetails_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackItemsRemaining', 'tap', function(e) {
                self.buttonBackItemsRemaining_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackItemRemSetting', 'tap', function(e) {
                self.buttonBackItemRemSetting_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackItemsSerial', 'tap', function(e) {
                self.buttonBackItemsSerial_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackPick', 'tap', function(e) {
                self.buttonBackPick_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackShippingAddress', 'tap', function(e) {
                self.buttonBackShippingAddress_tap(e);
            });
            Shared.AddRemoveHandler('#buttonBackSkippedItems', 'tap', function(e) {
                self.buttonBackSkippedItems_tap(e);
            });
            Shared.AddRemoveHandler('#buttonSkippedItemsSetting', 'tap', function(e) {
