    ...
    function (
        ...
        var PickView = Backbone.View.extend({
            ...
            buttonBackBoxDetails_tap: function (e) {
                ...
                if (this.CurrentItem.get('CarrierDescription') == "Manual") this
                    .UpdateManualBoxInfo();
            },
            buttonBackShippingAddress_tap: function (e) {
                ...
                if (this.ValidateShippingAddress(this.CurrentItem)) {
                    ...
                    this.$('#buttonAddress').removeClass("btn-danger").addClass(
                        "btn-inverse2");
                } else {
                    this.$('#textAddress').text("incomplete shipping address");
                    this.$('#buttonAddress').addClass("btn-danger").removeClass(
                        "btn-inverse2");
                }
                this.SwitchPage("page1", null);
            },
            buttonMore_tap: function (e) {
                var self = this;
                this.$('#containerCompletedAnimation').removeClass(
                    'section-show').removeClass("bounceInDown").addClass(
                        "bounceOutUp");
                setTimeout(function () {
                    self.GoToLookup();
                }, 300);
            ...
            buttonPackOrder_tap: function (e) {
                if (Preference.PickSourceTransaction == "order") {
                    if (Preference.PickIsAutoComplete) {
                        if (Preference.PackSourceTransaction == "invoice") window.location
                            .hash = "pack";
                        else Shared.NotifyError(
                            "The action is not allowed since packing config does not support invoice."
                        );
                    } else {
                        if (Preference.PackSourceTransaction == "order") window.location
                            .hash = "pack";
                        else Shared.NotifyError(
                            "The action is not allowed since packing config does not support order."
                        );
                    }
                } else {
                    if (Preference.PackSourceTransaction == "invoice") window.location
            ...
            numericpad_webkitTransitionEnd: function (e) {
                if ($('.numericpad').hasClass('slideInUp')) { } else {
                    if (this.NumericPadType == "freightrate") return true;
                    if (this.CurrentItem) {
                        if (isSkipped) {
                            this.RemoveFromSkippedItemList(this.CurrentItem, this.QuantityToScan);
                        } else {
                            this.PickNextItem(this.CurrentItem, this.QuantityToScan);
                        }
                    }
            ...
            textManualTrackingNumber_keyup: function (e) {
                if (!$('#textManualTrackingNumber').val()) {
                    $('#textManualTrackingNumber').parent().css('color',
                        '#d2322d');
                } else {
                    $('#textManualTrackingNumber').parent().css('color',
                        '#2c3e50');
            ...
            AnimatePick: function (isAnimate, isFullScreen) {
                this.ProgressScreen = Shared.ShowProgress(this.ProgressScreen,
                    isAnimate, "containerPick", isFullScreen);
            },
            ...
            ChangeCardSize: function (skippedItemID) {
                if (Global.IsPrePackMode) {
                    this.$('#pickItem' + skippedItemID).addClass(
                        "pick-item-prepack");
                    this.$('#pickItemDetail' + skippedItemID).addClass(
                        "pick-item-detail-prepack");
                    this.$('#pickItemImage' + skippedItemID).addClass(
                        "imageCard-prepack");
                    this.$('#itemSettings' + skippedItemID).addClass(
                        "pick-item-prepack");
                    try {
                        var detailScroll = new IScroll('#itemSettingBody' +
                            skippedItemID, {
                            vScrollbar: false,
                            scrollY: true
                        });
                    ...
                    pickModel.save(null, {
                        success: function (model, response, options) {
                            if (!model.get("hasError")) {
                                if (response != null && response.TransactionCodes !=
                                    null && response.TransactionCodes.length > 0) {
            ...
            CreateShipment: function (item, qty) {
                if (item) {
                    ...
                    var isInvoice = (Preference.PickSourceTransaction.toLowerCase() ==
                        "invoice");
                    pickModel.set({
                    ...
                    pickModel.save(null, {
                        success: function (model, response, options) {
                            if (!model.get("hasError")) {
                                if (Preference.PickIsAutoComplete) {
                                    if (response.TransactionCodes && response.TransactionCodes
                                        .length > 0) self.PrepackTransactionCode =
                                            response.TransactionCodes[0];
                                }
                                self.PrintLabel(response);
                            } else {
                                navigator.notification.alert(
                                    "You may retry repacking the order either in CB or in CW.",
                                    null, "Pack Failed", "OK");
                                var carrierDesc = self.CurrentItem.get(
                                    'CarrierDescription');
            ...
            CheckSerialExist: function (serialLotNumber, currentItem) {
                ...
                itemLookup.url = Global.ServiceUrl + Service.PRODUCT + Method.ISSERIALEXIST +
            ...
            InitializeModel: function () {
                ...
                if (Global.CurrentTransactions != null && Global.CurrentTransactions
                    .models.length > 0) {
                    this.model.set({
                        BillToCode: Global.CurrentTransactions.models[0].get(
                            "BillToCode"),
                        ShippingDate: Global.CurrentTransactions.models[0].get(
                            "ShippingDate"),
                        ShiptoCode: Global.CurrentTransactions.models[0].get(
                            "ShiptoCode")
                    });
                } else {
                    this.model.set({
            ...
            InitializeBoxDetails: function () {
                if (this.CurrentItem) {
                    ...
                    var shippingMethod = this.CurrentItem.get(
                        'ShippingMethodCode');
                    ...
                    if (carrierDesc == 'Manual') {
                        ...
                        this.$('#textFreightRate').val(this.CurrentItem.get(
                            "FreightRate"));
                    ...
                    this.$('#textBoxWeight').text(this.CurrentItem.get(
                        'WeightInLbs'));
                    this.$('#textDimension').text(this.CurrentItem.get(
                        'Dimensions'));
            ...
            InitializeItemLookup: function () {
                ...
                this.ItemLookupView.on("itemSelected", function (item) {
                    self.PickItemFromItemLookup(item);
                });
            },
            ...
            InitializeNumericPad: function () {
                ...
                view.on('closenumericpad', function (e) {
                    ...
                    // git issue #33 -- changed by sandeep 3 september 2021
            ...
            InitializeItemSetting: function () {
                ...
                if (isFromRemainingItemSection) {
                    $('#remSettingsItemCode').text(this.CurrentItem.get(
                        'ItemCode'));
                    $('#remSettingsItemName').text(this.CurrentItem.get(
                        'ItemName'));
                    //$('#remSettingsItemDesc').text(this.CurrentItem.get('ItemDescription'));
                    $('#remSettingsUPCCode').text(Shared.ConvertNullToEmptyString(
                        this.CurrentItem.get('UPCCode')));
                    $('#remSettingsQuantity').text(this.CurrentItem.get(
                        'QuantityToPick'));
                    $('#remSettingsFreeStock').text(this.CurrentItem.get(
                        'FreeStock'));
                    $('#remSettingsRemUnitMeasureCode').text(this.CurrentItem.get(
                        'UnitMeasureCode'));
                    $('#remSettingsBinLocation').text(this.CurrentItem.get(
                        'BinLocationName'));
                    $('#remSettingsTransactionCode').text(this.CurrentItem.get(
                        'TransactionCode'));
                    ...
                    if (shippingNotes) {
                        ...
                        if (shippingNotes.length > 20) {
                            this.$('#rowRemShippingNotes').html(
                                "<a style='color:white!important;'>Notes<br><span style='white-space:normal'>" +
                                shippingNotes + "</span></a>")
                        } else {
                            this.$('#rowRemShippingNotes').html(
                                "<a style='color:white!important;'><span class='pull-right'>" +
                                shippingNotes + "</span>Notes</a>")
                        }
                    } else $('#rowRemShippingNotes').hide();
                    if (itemDescription.length > 20) {
                        this.$('#rowRemItemDesc').html(
                            "<a style='color:white!important;'>Description<br><span style='white-space:normal'>" +
                            itemDescription + "</span></a>")
                    } else {
                        this.$('#rowRemItemDesc').html(
                            "<a style='color:white!important;'><span class='pull-right'>" +
                            itemDescription + "</span>Description</a>")
                    }
                }
            },
            ...
            InitializeSerialCollection: function () {
                ...
                if (SerialNumberTransactionValidationType == null ||
                    SerialNumberTransactionValidationType <= 1 ||
                    SerialNumberTransactionValidationType > 3) {
            ...
            IsItemSkippedExisting: function () {
                ...
                var existingItem = Shared.FindItem(this.itemsSkipped, itemCode,
                    umCode);
            ...
            GoToSelectedItem: function (itemView) {
                ...
                if (itemView.model.get('Quantity') > 1) skippedListItemID =
                    skippedListItemID + "B";
                var listItem = $('#' + skippedListItemID).parent();
                var skippedItemPosition;
                if (listItem.length > 0) skippedItemPosition = this.$(
                    '#skippedItemList > li').index(listItem) * (Global.ScreenWidth *
                        -1);
                else skippedItemPosition = ((this.$('#skippedItemList > li').length -
                    1) * (Global.ScreenWidth * -1));
            ...
            HighLightListItem: function (itemView) {
                this.$('.listitem-1').addClass('listitem-1').removeClass(
                    'listitem-2');
                this.$('.listitem-2').addClass('listitem-1').removeClass(
                    'listitem-2');
                Shared.HighLightItem(itemView.cid, "CheckList");
                this.$('#listItem' + itemView.model.get('ViewID')).removeClass(
                    'listitem-1').addClass('listitem-2');
            },
            HighLightServiceType: function (itemView) {
                this.$('.listitem-2').addClass('listitem-7').removeClass(
                    'listitem-2');
                this.$('.listitem-2').addClass('listitem-7').removeClass(
                    'listitem-2');
                Shared.HighLightItem(itemView.cid, "CheckList");
                this.$('#listItem' + itemView.model.get('ViewID')).removeClass(
                    'listitem-2').addClass('listitem-7');
            },
            LoadCheckListLookup: function (listMode) {
                ...
                switch (listMode) {
                    case Enum.ListMode.Boxes:
                        ...
                        if (!this.CurrentItem.get("CarrierCode") || this.CurrentItem
                            .get("CarrierCode") == "") isShowAllBoxes = true;
                        lookupModel.url = Global.ServiceUrl + Service.SOP + Method.CARRIERPACKAGINGLOOKUP;
                        lookupModel.set({
                            CarrierCode: this.CurrentItem.get("CarrierCode"),
                            ShippingMethodCode: this.CurrentItem.get(
                                "ShippingMethodCode"),
                            IsShowAllBoxes: isShowAllBoxes
                        });
                        break;
                    case Enum.ListMode.Carriers:
                        lookupModel.url = Global.ServiceUrl + Service.CUSTOMER +
                            Method.CARRIERLOOKUP + "30";
                        break;
                    case Enum.ListMode.ServiceTypes:
                        lookupModel.url = Global.ServiceUrl + Service.CUSTOMER +
                            Method.SERVICETYPELOOKUP + "30";
                        if (this.CurrentItem.get("CarrierDescription") == "Manual")
                            lookupModel.set({
                                CarrierCode: this.CurrentItem.get(
                                    "DescriptionCarrierCode")
                            });
                        else lookupModel.set({
                            CarrierCode: this.CurrentItem.get("CarrierCode")
                        });
                        break;
                }
            ...
            LoadiScrollServiceType: function (myScroll) {
                if (myScroll) {
                    ...
                    return new IScroll('#containerServiceTypeCheckList', {
                        scrollY: true,
                        scrollbars: true,
                    });
                }
            ...
            PickItemFromItemLookup: function (item) {
                ...
                var existingItem = Shared.FindItem(this.itemCollection,
                    itemCode, unitMeasureCode);
                if (existingItem) {
                    if (this.CurrentItem.get('UPCCode') == upcCode && this.CurrentItem
                        .get('UnitMeasureCode') == unitMeasureCode) {
                        Shared.BeepSuccess();
            ...
            PopulateCart: function (items) {
                if (items && items.length > 0) {
                    ...
                    _.each(items, function (current) {
                        ...
                        if (current.UPCCode != null && current.UPCCode != "")
                            card.set({
                                BarCode: current.UPCCode
                            })
                        else card.set({
                        ...
                        card.set({
                            ...
                            CardImage: Shared.GetImageUrl(Enum.ImageType.Card,
                                current.ItemMediumFile),
                            CartItemImage: Shared.GetImageUrl(Enum.ImageType.CartItem,
                                current.ItemIconFile),
                        ...
                        if (Global.IsPrePackMode) {
                            ...
                            if (current.ShipToAddressType == null || current.ShipToAddressType ==
                                "") current.ShipToAddressType = "Residential";
                            card.set({
                        ...
                        if (card.get("ItemType") == "Kit") {
                            ...
                            self.$("#cartListItemsRemaining tbody").append(
                                cartItemView.render());
                    ...
                    this.model.set({
                        OverallCounter: totalQty
                    });
                    this.UpdateCounter(totalQty);
                }
            },
            PrepareBin: function (items) {
                if (items && items.length == 1 && items[0].QuantityToPick == 1)
                    this.model.set({
                        HasOnlyOneItem: true
                    });
                if (items && items.length > 0) {
                    ...
                    navigator.notification.alert(
                        "There are no items to pick. Please select another order",
                        this.GoToLookup(), "No Pick Items", "OK");
                }
            },
            ProcessPickItems: function (item, quantityToScan) {
                if (this.itemCollection && this.itemCollection.length > 0) {
                    ...
                    if (Global.IsPrePackMode) {
                        ...
                        if (quantityToScan > 0) {
                            Shared.BeepSuccess();
                            // git issue #24 -- changed by sandeep 8 july 2021
                            item.set({
                                RemainingQuantity: 0
                            });
            ...
            ProcessCurrentPickItem: function (item) {
                ...
                item.set({
                    QuantityToPick: maxQty - quantityToScan
                });
                if (remainingQty <= 0) {
                    //
                    ...
                    if (this.itemCollection.length == 1 && item.get(
                        'QuantityToPick') == 1 && maxQty == 1) this.HideSkipButton(
                            item.get('SkippedListItemID'));
                }
                this.UpdateCounter(quantityToScan * -1);
                item.set({
                    OverallCounter: this.model.get("Counter")
            ...
            ProcessPrePackItem: function (item, quantityToScan) {
                ...
                if (!this.ValidateShippingAddress(item)) {
                    hasError = true;
                    Shared.NotifyError(
                        "Please fill up complete shipping address.");
                }
                ...
                if (this.CurrentItem.get('ServiceCode') == "(Unspecified)") {
                    hasError = true;
                    Shared.NotifyError(
                        "(Unspecified) Service Type is not yet supported.");
                }
                if (this.CurrentItem.get('CarrierDescription') == "Manual") {
            ...
            ProcessSkipItems: function () {
                if (this.itemCollection && this.itemCollection.length > 0) {
                    ...
                    if (itemModel != undefined) {
                        var itemNameModelName = this.ItemSettingView.model.get(
                            "ItemName");;
            ...
            PickNextItem: function (item, quantityToScan) {
                if (this.IsValidPickTransaction()) {
                    ...
                    if (SerialNumberTransactionValidationType > 1 && item.get(
                        "SerializeLot") == "Serial" && Global.IsPrePackMode ==
                        false) {
                        var maxQty = 0
                        if (Preference.PickIsPromptForQty) maxQty = item.get(
                            "QuantityToPick");
            ...
            IsValidPickTransaction: function () {
                if (Preference.PickSourceTransaction == "invoice" && Preference
                    .PickIsAutoComplete) return true;
                else if (Preference.PickSourceTransaction == "invoice" && !
                    Preference.PickIsAutoComplete) return true;
                else if (Preference.PickSourceTransaction == "order" &&
                    Preference.PickIsAutoComplete) return true;
                else if (Preference.PickSourceTransaction == "order" && !
                    Preference.PickIsAutoComplete) return false;
                else return true;
                return true;
            },
            PickCompleted: function (transactionCode) {
                ...
                if (!Global.IsPrePackMode) {
                    ...
                    if (Preference.PickSourceTransaction == "order") {
                        if (Preference.PickIsAutoComplete) this.$(
                            '#buttonPackOrder').text('pack invoice');
                        else this.$('#buttonPackOrder').text('pack order');
                    } else this.$('#buttonPackOrder').text('pack invoice');
                } else {
                    if (Preference.PickIsAutoComplete) this.$(
                        '#completedNavTitle').text(transactionCode);
                }
                this.$("#containerCompletedAnimation").removeClass(
                    "bounceOutUp").show().addClass("bounceInDown");
                Shared.SetVisibilityToHidden("#pickHeader");
            },
            PrintLabel: function (response) {
                if (response != null) {
                    if (Preference.PrePackIsAutoPrint && this.CurrentItem.get(
                        'CarrierDescription') != "Manual") {
                        var self = this;
                        this.AnimatePick(true, true);
                            ...
                            if (list.Packages && list.Packages.length > 0) {
                                if (self.CurrentItem.get('CarrierCode')) {
                                    _.each(list.Packages, function (current) {
                                        ...
                                        var checklistItemView = new CheckListItemView({
                                            model: checkListItemModel
                                        });
                                        self.$('#cartCheckList tbody').append(
                                            checklistItemView.render());
                                        if (self.CurrentItem.get('PackagingCode') ==
                                            checklistItemView.model.get('PackagingType')) {
                                            self.HighLightListItem(checklistItemView);
                                        }
                                        Shared.AddRemoveHandler('.' + checklistItemView
                                            .cid, 'tap',
                                            function (e) {
                                                self.HighLightListItem(checklistItemView);
                                                self.UpdatePackagingType(
                                                    checklistItemView);
                                                self.SwitchPage('page2');
                                            });
                                    });
                                    this.iScrollCheckList = Shared.LoadiScroll(this.iScrollCheckList,
                                        "CheckList");
                                } else {
                                    ...
                                    if (list.Carriers && list.Carriers.length > 0) {
                                        _.each(list.Carriers, function (current) {
                                            ...
                                            var checkListGroupView = new CheckListGroupView({
                                                model: checkListGroupModel
                                            });
                                            var tableName = '#' + current.CarrierDescription +
                                                'Table';
                                            var iconName = current.CarrierDescription +
                                                'Icon';
                                            self.$('#serviceTypeCheckList').append(
                                                checkListGroupView.render());
                                            Shared.AddRemoveHandler(tableName + ' thead',
                                                'tap',
                                                function (e) {
                                                    self.ShowServiceTypeRows(tableName,
                                                        iconName);
                                                });
                                        });
                                    }
                                    if (list.Packages && list.Packages.length > 0) {
                                        _.each(list.Packages, function (current) {
                                            ...
                                            self.$('#' + current.CarrierDescription +
                                                'Body').append(checklistItemView.render());
                                            $('.' + checklistItemView.cid).hide();
                                            $('.' + checklistItemView.cid + ' div ul').removeClass(
                                                'listitem-1').addClass('listitem-2');
                                            Shared.AddRemoveHandler('.' +
                                                checklistItemView.cid, 'tap',
                                                function (e) {
                                                    self.HighLightServiceType(
                                                        checklistItemView);
                                                    self.UpdatePackagingType(
                                                        checklistItemView);
                                                    self.SwitchPage('page2');
                                    ...
                                    this.iScrollServiceType = self.LoadiScrollServiceType(
                                        this.iScrollServiceType);
                                }
                            }
                            break;
                        case Enum.ListMode.Carriers:
                            if (list.Packages && list.Packages.length > 0) {
                                _.each(list.Packages, function (current) {
                                    ...
                                    if (self.$('#textCarrier').text() == 'Manual') {
                                        if (checklistItemView.model.get(
                                            'CarrierDescription') == 'Manual') self.HighLightListItem(
                                                checklistItemView);
                                    } else {
                                        if (self.CurrentItem.get('CarrierCode') ==
                                            checklistItemView.model.get('CarrierCode'))
                                            self.HighLightListItem(checklistItemView);
                                    }
                                    Shared.AddRemoveHandler('.' + checklistItemView.cid,
                                        'tap',
                                        function (e) {
                                            self.HighLightListItem(checklistItemView);
                                ...
                                if (self.CurrentItem.get('CarrierCode')) {
                                    _.each(list.Packages, function (current) {
                                        if (current.ServiceType != '(Unspecified)') {
                                            ...
                                            self.$('#cartCheckList tbody').append(
                                                checklistItemView.render());
                                            if (self.CurrentItem.get('ServiceCode') ==
                                                checklistItemView.model.get('ServiceType')) {
                                                self.HighLightListItem(checklistItemView);
                                            }
                                            Shared.AddRemoveHandler('.' +
                                                checklistItemView.cid, 'tap',
            ...
            RenderItem: function (item, isAnimate) {
                ...
                if (isAnimate) {
                    ...
                    var cardNumber = this.model.get("OverallCounter") - this.model
                        .get("RemainingQuantity") + 1;
                    item.set({
            ...
            RenderItemDetail: function (item) {
                ...
                var binLocationCard = $('#textbinLocation' + item.get(
                    "ItemCode"));
                var binLocationCardDetail = $('#textbinLocationDetail' + item.get(
                    "ItemCode"))
                ...
                var onAttributeChanged = function () {
                    ...
                    var binLocationCard = $('#textbinLocation' + item.get(
                        "ItemCode"));
                    var binLocationCardDetail = $('#textbinLocationDetail' +
                        item.get("ItemCode"))
                    ...
                    if (binLocationItemSetting.text() != newBinLocation)
                        binLocationItemSetting.text(newBinLocation);
                };
            ...
            ScanSelectedItem: function (itemView) {
                ...
                if (!Preference.PickIsAllowToSkipItems && currentItemName !=
                    itemName) {
                    Shared.NotifyError(
                        "Preference does not allow skipping an item.");
                    Shared.BeepError();
                } else {
                    ...
                    var selectedItem = Shared.FindItem(this.itemCollection,
                        itemCode, unitMeasureCode);
                ...
                if (this.itemCollection && this.itemCollection.length > 0) {
                    this.itemCollection.each(function (item) {
                        ...
                        if (quantityToPick != 0) {
                            ...
                            Shared.AddRemoveHandler('#buttonItemSetting' +
                                itemView.model.get('TapeItemID'), 'tap',
                                function (event) {
                                    self.LoadItemSetting(itemView)
                                    $("#itemRemainingSlider").removeClass(
                                        "container-remaining slideInUp").addClass(
                                            "container-remaining slideOutDown");
                                    event.stopPropagation();
                                });
                            Shared.AddRemoveHandler('#' + itemView.model.get(
                                'TapeItemID'), 'tap', function () {
                                    self.ScanSelectedItem(itemView);
                                });
                            if (!Global.IsPrePackMode) $('#rowTransactionCode' +
                                itemView.model.get('TapeItemID')).hide();
                        };
                    });
                };
            },
            RenderSkippedItems: function () {
                ...
                if (totalItemsSkipped > 0) {
                    ...
                    this.itemsSkipped.each(function (item) {
                        ...
                        var cartSkippedItemView = new CartSkippedItemView({
                            model: item
                        });
                       
                        if (cartSkippedItemView.model.get('QuantitySkipped') >
                            0 || !cartSkippedItemView.model.get('IsItemSkipped')) {

                                console.log("Skipped Items",cartSkippedItemView.model);
                           
                                // git issue #24 -- changed by sandeep 29 Dec 2021
                                // self.$("#cartListSkippedItems tbody").html(cartSkippedItemView.render());
                               self.$("#cartListSkippedItems tbody").append(cartSkippedItemView.render());

                            $('#itemRow' + cartSkippedItemView.model.get(
                                'SkippedListItemID')).text(rownumber);
                            if (cartSkippedItemView.model.get("UPCCode") == self.CurrentItem
                                .get("UPCCode")) Shared.HighLightItem(
                                    cartSkippedItemView.model.get('SkippedListItemID'),
                                    "Default");
                            rownumber += 1;
                            Shared.AddRemoveHandler('#skippedItemRow' +
                                cartSkippedItemView.model.get('SkippedListItemID'),
                                'tap',
                                function () {
                                    Shared.HighLightItem(cartSkippedItemView.model.get(
                                        'SkippedListItemID'), "Default");
                                    $("#skippedItemsSlider").removeClass(
                                        "container-remaining slideInUp").addClass(
                                            "container-remaining slideOutDown");
                                    setTimeout(function () {
                                        self.GoToSelectedItem(cartSkippedItemView)
                                    }, 300);
                                });
                        }
                        if (!cartSkippedItemView.model.get('IsItemSkipped')) {
                            $('#itemQty' + cartSkippedItemView.model.get(
                                'SkippedListItemID')).text(item.get(
                                    'RemainingQuantity'));
                        }
                    });
                    this.itemsSkipped.models.reverse();
            ...
            RenderSkippedItemList: function (item) {
                ...
                if (item.get('Quantity') > 1) {
                    ...
                    item.set({
                        SkippedListItemID: skippedItemID
                    });
                }
                ...
                this.$('#skippedItemList #currentCardContainer').before(
                    cardListView.render());
                var cardCoverView = new CardView({
                    model: item
                });
                var itemSettingView = new ItemSettingView({
                    model: item
                });
                $('#containerCard' + skippedItemID).append(cardCoverView.render());
                $('#containerItemSettings' + skippedItemID).append(
                    itemSettingView.render());
                this.ChangeCardSize(skippedItemID);
                this.WireCardEvents(skippedItemID, true);
                this.ShowFreeStockItemSetting(skippedItemID);
                this.ShowTransactionCodeItemSetting(skippedItemID);
                $('#textQty' + skippedItemID).text(item.get("QuantitySkipped"));
                $('#textQtyDetail' + skippedItemID).text(item.get(
                    "QuantitySkipped"));
                if (binLocationName == 'No Bin') {
                    var binLocationCard = $('#textbinLocation' + item.get(
                        "ItemCode"));
                    var binLocationCardDetail = $('#textbinLocationDetail' + item
                        .get("ItemCode"))
                    binLocationCard.css("font-style", "italic");
                    binLocationCardDetail.css("font-style", "italic");
                    binLocationCard.text("< No Bin >");
                ...
                item.set({
                    SkippedListItemID: tempSkippedItemID
                });
            ...
            RemoveFromSkippedItemList: function (item, quantityToScan) {
                ...
                if (qtySkipped <= 0) {
                    ...
                    if (skipListControl.length == 0) {
                        skipListControl = this.$('#' + skippedItemID + "B");
                    }
                    ...
                    $('#textQty' + item.get('SkippedListItemID') + "B").text(
                        qtySkipped);
                    $('#textQtyDetail' + item.get('SkippedListItemID') + "B").text(
                        qtySkipped);
                }
                if (Global.IsPrePackMode) {
                    item.set({
            ...
            ShowHideShippingNotes: function (shippingNotes, skippedItemId) {
                if (shippingNotes) {
                    $('#rowShipNotes').show();
                    if (shippingNotes.length > 20) {
                        this.$('#rowShipNotes' + skippedItemId).html(
                            "<a style='color:white!important;'>Notes<br><span style='white-space:normal'>" +
                            shippingNotes + "</span></a>")
                    } else {
                        this.$('#rowShipNotes' + skippedItemId).html(
            ...
            RotateContainerCard: function (skippedListItemID) {
                if (isRotateCard) {
                    if (isOnItemSettingSection) {
                        ...
                        var skippedItemID = this.CurrentItem.get(
                            "SkippedListItemID");
                        var itemDescription = this.CurrentItem.get(
                            'ItemDescription');
                        var shippingNotes = this.CurrentItem.get('ShippingNotes');
                        if (itemDescription.length > 20) {
                            this.$('#rowDescription' + skippedItemID).html(
                                "<a style='color:white!important;'>Description<br><span style='white-space:normal'>" +
                                itemDescription + "</span></a>")
                        } else {
                            this.$('#rowDescription' + skippedItemID).html(
            ...
            SetShippingAddress: function (item) {
                if (item) {
                    ...
                    if (this.$('#textPlus4').val() == "") item.set({
                        ShipToPlus4: 0
                    })
                    else item.set({
                        ShipToPlus4: parseInt(this.$('#textPlus4').val())
                    })
                }
            },
            SlideDownLookup: function () {
                $('#lookupSection').addClass('slideOutDown').removeClass(
                    'slideInUp');
            },
            PickedScanItem: function (e) {
                ...
                if (itemName != valueToCheck) {
                    var unitMeasureCode = this.ItemSettingView.model.get(
                        'UnitMeasureCode');
                    var selectedItem = Shared.FindItem(this.itemCollection, e,
                        unitMeasureCode);
                    this.CurrentItem = selectedItem;
                    //this.$("#textScanItem").val(itemCode);
                    this.$("#textScanItem").val('');
                    if (Preference.PickIsPromptForQty) {
                        this.ShowNumericPad();
                    } else this.PickNextItem(selectedItem, 1);
                    //this.ShowRemainingItems(true);
                } else if (valueToCheck == upcCode || valueToCheck == itemCode ||
                    valueToCheck == itemName) {
                    if (valueToCheck == upcCode || valueToCheck == itemCode ||
                        valueToCheck == itemName) {
                        if (isOnItemSettingSection) {
                        ...
                        if (Preference.PickIsPromptForQty) {
                            ...
                            if (qtySkipped > 0) this.RemoveFromSkippedItemList(this.CurrentItem,
                                1);
                            else this.PickNextItem(this.CurrentItem, 1);
                        }
                    }
            ...
            ScanSerial: function (valueToCheck) {
                ...
                if (Preference.PreventDuplicateSerialNumber) {
                    ...
                    serialQuantity += 1;
                    $("#serialScanned").text(serialQuantity);
                    if (serialQuantity < serialRequired) Shared.Focus(
                        "#textScanSerial");
            ...
            ShowBinManager: function () {
                ...
                this.BinManagerView.model.set({
                    ItemCode: this.CurrentItem.get('ItemCode')
                });
                this.BinManagerView.CurrentItem = this.CurrentItem;
                var itemModel = this.CurrentItem;
                isRotateCard = false;
                this.$("#binManagerSlider").removeClass(
                    "container slideOutDown").addClass("container slideInUp");
                this.BinManagerView.on("selectedBin", function (item) {
                    self.UpdateBinItemSortOrder(item);
                });
                this.BinManagerView.on("deleteBin", function (binCode,
                    binCollection, isDeleteAll) {
                    Shared.RemoveBinFromItem(itemModel, binCode,
                        binCollection, isDeleteAll);
                });
            ...
            ShowServiceTypeRows: function (tableName, iconName) {
                ...
                if ($('#' + iconName).hasClass('icon-rotate-180')) $('#' +
                    iconName).removeClass('icon-rotate-180').addClass(
                        'icon-rotate-0')
                else $('#' + iconName).addClass('icon-rotate-180').removeClass(
                    'icon-rotate-0')
            ...
            ShowShippingDetailsLookup: function (shippingMode) {
                ...
                if (shippingMode == "postal") lookupView.CountryCode = self.CurrentItem
                    .get("ShipToCountry");
                ...
                $('#lookupSectionContainer').removeClass('section-close').addClass(
                    'section-show');
                $('#completedSection').removeClass('section-show').addClass(
                    'section-close');
                lookupView.InitializeChildViews();
                $('#lookupSection').addClass('slideInUp').removeClass(
                    'slideOutDown');
            },
            ...
            ShowHideControls: function () {
                isRotateCard = true;
                if (Preference.PickIsAllowToSkipItems) this.$('#buttonSkipItem')
                    .show();
            ...
            ShowItemLookup: function (items) {
                if (items) {
                    ...
                    $('#pickHeader').hide();
                    $('#itemLookupSection').addClass('slideInUp').removeClass(
                        'slideOutDown');
                }
            },
            ShowNumericPad: function () {
                if (SerialNumberTransactionValidationType > 1 && this.CurrentItem
                    .get("SerializeLot") == "Serial") {
                ...
                if (this.NumericPadType == "freightrate") {
                    ...
                    $('#Spndes').text(texta);
                    //------------------------------------------------------------------------------
                    $('#textboxQuantity').val(this.CurrentItem.get(
                        'QuantityToPick'));
                    localStorage.QuantityToPick = this.CurrentItem.get(
                        'QuantityToPick');
                }
                this.$('#textScanItem').blur();
                $('#pickHeader').hide();
                $('.numericpad').addClass('slideInUp').removeClass(
                    'slideOutDown');
            },
            ShowRemainingItems: function (isShow) {
                if (isShow) {
                    this.$('#textScanItem').blur();
                    $("#itemRemainingSlider").removeClass(
                        "container-remaining slideOutDown").addClass(
                            "container-remaining slideInUp");
                    this.itemRemainingiScroll = Shared.LoadiScroll(this.itemRemainingiScroll,
                        "ItemsRemaining");
            ...
            ProcessSerializeItem: function () {
                var self = this;
                if (self.CurrentItem) {
                    if (isSkipped) self.RemoveFromSkippedItemList(self.CurrentItem,
                        self.QuantityToScan);
                    else self.ProcessPickItems(self.CurrentItem, serialRequired);
                }
                $("#serialSlider").removeClass("container-remaining slideInUp")
                    .addClass("container-remaining slideOutDown");
                Shared.Focus('#textScanItem');
            },
                ...
                if (isShow) {
                    ...
                    if (self.ValidateSerial()) {
                        if (SerialNumberTransactionValidationType == 0 ||
                            SerialNumberTransactionValidationType == 2) {
                ...
                if (totalItemsSkipped > 0) {
                    ...
                    if (isShow) {
                        $("#skippedItemsSlider").removeClass(
                            "container-remaining slideOutDown").addClass(
                                "container-remaining slideInUp");
                        this.skippedItemsiScroll = Shared.LoadiScroll(this.skippedItemsiScroll,
                            "SkippedItems");
            ...
            SkipItem: function () {
                ...
                this.itemSkippediScroll = Shared.LoadHorizontaliScroll(
                    '#pickBody', this.itemSkippediScroll);
                this.itemSkippediScroll.on("scrollEnd", function (e) {
                    self.GetCurrentItem();

                    // change by sandeep support@dynenttech.com git issue #49 14-sep-2021
                    setTimeout(function () {
                        $('#skippedItemList #currentCardContainer').removeClass('disabled');
                    }, 900);
                ...
                // change by sandeep support@dynenttech.com git issue #49 14-sep-2021
                $('#skippedItemList #currentCardContainer').addClass('disabled');
                //Shared.Focus('#textScanItem');
            },
            SwitchDisplay: function (page) {
                if (this.isAnimate) return false;
                $('.slideInUp').css('-webkit-transition',
                    'all 0.3s ease-in-out 0.1s');
                switch (page) {
                    case "page1":
                        if (isFromRemainingItemSection) Shared.SlideX(
                            '#itemRemainingSlider', 0);
                        else Shared.SlideX('#skippedItemsSlider', 0);
                        break;
                    case "page2":
                        if (isFromRemainingItemSection) Shared.SlideX(
                            '#itemRemainingSlider', Global.ScreenWidth * -1);
                        else Shared.SlideX('#skippedItemsSlider', Global.ScreenWidth *
                            -1);
                        break;
                    case "page3":
                        if (isFromRemainingItemSection) Shared.SlideX(
                    ...
                    case "page2":
                        ...
                        switch (feature) {
                            case "shippingAddress":
                                ...
                                this.$('#shippingAddressSection').removeClass(
                                    'section-close');
                                this.$('#boxDetailsSection').addClass('section-close');
                                break;
                            case "boxDetails":
                                this.InitializeBoxDetails();
                                this.$('#boxDetailsSection').removeClass(
                                    'section-close');
                                this.$('#shippingAddressSection').addClass(
                                    'section-close');
                                break;
                        }
                        $('li').removeClass('highlight');
                        Shared.SlideX('#pickContainerSlider', Global.ScreenWidth *
                            -1);
                        break;
                    case "page3":
                        Shared.SlideX('#pickContainerSlider', Global.ScreenWidth *
            ...
            UpdateCarrier: function (itemModel) {
                if (itemModel.model.get('CarrierDescription') == "Manual") {
                    ...
                    this.$("#rowFreightRate").show();
                    this.$('#rowServiceType').hide();
                    this.$('#textTrackingNumber').val(this.CurrentItem.get(
                        "TrackingNumber"));
                    this.$('#textFreightRate').val(this.CurrentItem.get(
                        "FreightRate"));
                    this.ValidateManualShippingInfo();
            ...
            UpdateCountry: function (itemModel) {
                this.CurrentItem.set({
                    ShipToCountry: itemModel.CountryCode
                });
                this.$('#textCountry').val(itemModel.CountryCode);
            },
            UpdateManualBoxInfo: function () {
                var textManualTrackingNumber = this.$(
                    '#textManualTrackingNumber').val();
            ...
            UpdatePackagingType: function (itemModel) {
                ...
                this.CurrentItem.set({
                    PackagingCode: itemModel.model.get('PackagingType')
                });
                this.$('#textCarrier').text(this.CurrentItem.get(
                    'CarrierDescription'));
                this.$('#textPackagingType').text(this.CurrentItem.get(
                    'PackagingCode'));
            },
            ...
            UpdateServiceType: function (itemModel) {
                if (itemModel.model.get('CarrierDescription') == "Manual") this
                    .CurrentItem.set({
                        ServiceCode: null
                    });
                else {
                ...
                this.$('#textCarrier').text(this.CurrentItem.get(
                    'CarrierDescription'));
                this.$('#textServiceType').text(this.CurrentItem.get(
                    'ServiceCode'));
            },
            ...
            ValidateCancelPick: function () {
                ...
                var counter = (this.model.get("Counter")) + (this.model.get(
                    "TotalItemsSkipped"));
                if (!Global.IsPrePackMode) {
                    if (overallCounter == counter) {
                        var self = this;
                        navigator.notification.confirm(
                            "Do you want to cancel Pick?",
                            function (button) {
                                if (button == 1) {
                                    if (!Preference.PickIsAutoComplete) {
                                        navigator.notification.confirm(
                                            "Do you want to save Picked Items?",
                                            function (button) {
                                                if (button == 1) {
                                                    self.CompletePick();
                                                } else {
                    ...
                    if (overallCounter != counter) {
                        var self = this;
                        navigator.notification.confirm(
                            "Some items have been picked. Do you want to cancel Pick?",
                            function (button) {
                                if (button == 1) {
                                    if (!Preference.PickIsAutoComplete) {
                                        navigator.notification.confirm(
            ...
            ValidateCompletePick: function () {
                ...
                if (this.itemsSkipped.length > 0 && this.itemsPicked.length > 0) {
                    navigator.notification.confirm(
                        "Some items are skipped. Do you still want to complete pick?",
                        function (button) {
                                ...
                                navigator.notification.confirm(
                                    ...
                                    function (button) {
                                        if (button == 1) {
                                            ...
                                            //self.GoToLookup();
                                            //self.RenderSkippedItems();
                                            self.ShowSkippedItems(true);
                                            isOnItemSettingSection = false;
                                        }
                                    }, "Save your work?", "Yes,No");
                            }
            ...
            ValidateManualShippingInfo: function () {
                var result = true;
                if (!this.CurrentItem.get('TrackingNumber') || this.CurrentItem
                    .get('TrackingNumber') == "" || this.CurrentItem.get(
                        'TrackingNumber') == null) {
                    $('#textManualTrackingNumber').parent().css('color',
                        '#d2322d');
                    result = false;
                ...
                if (!this.CurrentItem.get('FreightRate') || this.CurrentItem.get(
                    'FreightRate') == "" || this.CurrentItem.get('FreightRate') ==
                    0) {
            ...
            ValidateSerial: function () {
                ...
                if (SerialNumberTransactionValidationType == 0) {
                    ...
                    if (serialRequired == serialQuantity) return true;
                        ...
                        navigator.notification.alert(
                            "Pick quantity should be equal to the number of Serials.",
                            null, "", "OK");
                        return false;
                    }
                    ...
                    if (serialQuantity < serialRequired) {
                        navigator.notification.confirm(
                            "Picked quantity is not equal to the number of Serials. Do you want to continue?",
                            function (button) {
                                if (button == 1) {
                                    confirm = true;
                                    self.ProcessSerializeItem();
            ...
            ValidateShippingAddress: function (item) {
                if (item.get("ShipToCountry") == "" || item.get("ShipToCountry") ==
                    null) {
                    $('#textCountry').parent().addClass('has-error');
                    return false;
                } else {
                ...
                if (item.get("ShipToPostalCode") == "" || item.get(
                ...
                if (item.get("ShipToCity") == "" || item.get("ShipToCity") ==
                    null) {
                    $('#textCity').parent().addClass('has-error');
                    return false;
                } else {
                ...
                //    $('#textCounty').parent().removeClass('has-error');
                //}
                if (item.get("ShipToState") == "" || item.get("ShipToState") ==
                    null) {
                    $('#textState').parent().addClass('has-error');
                    return false;
                } else {
                ...
                if (item.get("ShipToAddress") == "" || item.get("ShipToAddress") ==
                    null) {
                    $('#textAreaAddress').parent().addClass('has-error');
                    return false;
                } else {
                ...
                if (item.get("ShipToPhone") == "" || item.get("ShipToPhone") ==
                    null) {
                    $('#textPhone').parent().addClass('has-error');
                    return false;
                } else {
                ...
                if (item.get("ShipToAddressType") == "" || item.get(
            ...
            ValidateShippingInfo: function (item) {
                if (item.get("CarrierCode") == "" || item.get("CarrierCode") ==
                    null) {
                    return false;
                }
                if (item.get("CarrierDescription") != "Manual") {
                    if (item.get("ServiceCode") == "" || item.get("ServiceCode") ==
                        null) {
                        return false;
                    }
                }
                if (item.get("CarrierDescription") == "Manual") {
                    if (item.get("TrackingNumber") == "" || item.get(
                        "TrackingNumber") == null) {
                        return false;
                    }
                }
                if (item.get("ShippingMethodCode") == "" || item.get(
                    "ShippingMethodCode") == null) {
                    return false;
                }
                if (item.get("CarrierDescription") != "Manual") {
                    if (item.get("PackagingCode") == "" || item.get(
                        "PackagingCode") == null) {
                        return false;
                    }
                }
            ...
            WireCardEvents: function (skippedItemID, isSkippedItem) {
                ...
                if (isSkippedItem) {
                    ...
                    if (Preference.PickIsPromptBinManager) {
                        Shared.AddRemoveHandler('#binColIcon' + skippedItemID,
                            'tap',
                            function (e) {
                                self.buttonBinManager_tap(e);
                            });
                        Shared.AddRemoveHandler('#binCol' + skippedItemID, 'tap',
                            function (e) {
                                self.buttonBinManager_tap(e);
                            });
                    } else {
                    ...
                    //Rotate Card
                    Shared.AddRemoveHandler('#' + skippedItemID, 'tap', function () {
                        self.RotateContainerCard(skippedItemID);
                    });
                } else {
                    //Bin Manager
                    ...
                    //Rotate Card
                    if (!$._data($('#currentCardContainer > div').get(0),
                        'events')) {
                        Shared.AddRemoveHandler('#currentCardContainer > div',
                            'tap',
                            function () {
                                self.RotateContainerCard("");
                            });
                ...
                //Skip Item
                if (Preference.PickIsAllowToSkipItems && !this.model.get(
                    "HasOnlyOneItem")) {
                    Shared.AddRemoveHandler('#buttonSkipItem' + skippedItemID,
                        'tap',
                        function () {
                            self.SkipItem();
                        });
                ...
                if (localStorage.AllowToScan == "false") {
                    $('.btn-picked').show();
                    Shared.AddRemoveHandler('#buttonPicked' + skippedItemID,
                        'tap',
                        function () {
            ...
            WireEvents: function () {
                var self = this;
                Shared.AddRemoveHandler('#buttonAddress', 'tap', function (e) {
                    self.buttonAddress_tap(e);
                });
                Shared.AddRemoveHandler('#buttonEditDetails', 'tap', function (
                    e) {
                    self.buttonEditDetails_tap(e);
                });
                Shared.AddRemoveHandler('#buttonScanItem', 'tap', function (e) {
                    self.buttonScanItem_tap(e);
                });
                Shared.AddRemoveHandler('#buttonMenu', 'tap', function (e) {
                    self.buttonMenu_tap(e);
                });
                Shared.AddRemoveHandler('#buttonMore', 'tap', function (e) {
                    self.buttonMore_tap(e);
                });
                Shared.AddRemoveHandler('#buttonPackOrder', 'tap', function (e) {
                    self.buttonPackOrder_tap(e);
                });
                Shared.AddRemoveHandler('#buttonCounter', 'tap', function (e) {
                    self.buttonCounter_tap(e);
                });
                Shared.AddRemoveHandler('#buttonBackBoxDetails', 'tap',
                    function (e) {
                        self.buttonBackBoxDetails_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonBackItemsRemaining', 'tap',
                    function (e) {
                        self.buttonBackItemsRemaining_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonBackItemRemSetting', 'tap',
                    function (e) {
                        self.buttonBackItemRemSetting_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonBackItemsSerial', 'tap',
                    function (e) {
                        self.buttonBackItemsSerial_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonBackPick', 'tap', function (e) {
                    self.buttonBackPick_tap(e);
                });
                Shared.AddRemoveHandler('#buttonBackShippingAddress', 'tap',
                    function (e) {
                        self.buttonBackShippingAddress_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonBackSkippedItems', 'tap',
                    function (e) {
                        self.buttonBackSkippedItems_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonSkippedItemsSetting', 'tap',
                    function (e) {
                        self.buttonSkippedItemsSetting_tap(e);
                    });
                Shared.AddRemoveHandler('#buttonPicked', 'tap', function (e) {
                    self.buttonPicked_tap(e);
                });
                Shared.AddRemoveHandler('#buttonScanSerial', 'tap', function (e) {
                    self.buttonScanSerial_tap(e);
                });
                Shared.AddRemoveHandler('#serialClear', 'tap', function (e) {
                    self.buttonSerialClear_tap(e);
                });
                Shared.AddRemoveHandler('#linkResidential', 'tap', function (e) {
                    self.linkResidential_tap(e);
                });
                Shared.AddRemoveHandler('#linkCommercial', 'tap', function (e) {
                    self.linkCommercial_tap(e);
                });
                Shared.AddRemoveHandler('#textCountry', 'tap', function (e) {
                    self.textCountry_tap(e);
                });
                Shared.AddRemoveHandler('#textFreightRate', 'tap', function (e) {
                    self.textFreightRate_tap(e);
                });
                Shared.AddRemoveHandler('#textPostal', 'tap', function (e) {
                    self.textPostal_tap(e);
                });
                Shared.AddRemoveHandler('#buttonBackCheckList', 'tap', function (
                    e) {
                    self.buttonBackCheckList_tap(e)
                });
                Shared.AddRemoveHandler('#rowCarrierCode', 'tap', function (e) {
                    self.rowCarrierCode_tap(e);
                });
                Shared.AddRemoveHandler('#rowPackagingType', 'tap', function (e) {
                    self.rowPackagingType_tap(e);
                });
                Shared.AddRemoveHandler('#rowServiceType', 'tap', function (e) {
                    self.rowServiceType_tap(e);
                });
            }
        });
        return PickView;
